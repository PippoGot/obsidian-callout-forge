...
const MOCK_CONTEXT = { some: "context" } as any;

describe("CodeblockParser", () => {

    // New: verifies context is stored
    it("stores the passed rendering context", () => {
        const parser = new CodeblockParser(MOCKS.singleLineProperty, MOCK_CONTEXT);
        expect(parser.context).toBe(MOCK_CONTEXT);
    });

    // New: verifies static fromString works
    it("parses using fromString static method", () => {
        const pairs = CodeblockParser.fromString(MOCKS.singleLineProperty, MOCK_CONTEXT);
    ...
    it("parses multiline pair with plain text", () => {
        const parser = new CodeblockParser(MOCKS.multilineProperty, MOCK_CONTEXT);
        expect(parser.pairList).toHaveLength(2);
    ...
    it("parses pair with codefence", () => {
        const parser = new CodeblockParser(MOCKS.pairWithCodefence, MOCK_CONTEXT);
    ...
    it("parses multiple pairs and fences", () => {
        const parser = new CodeblockParser(MOCKS.multiplePropertiesAndFences, MOCK_CONTEXT);
    ...
    it("throws error if input is not a string", () => {
        // @ts-expect-error
        expect(() => new CodeblockParser(123, MOCK_CONTEXT)).toThrow("Source must be a string to parse.");
    });

    // Throws if input is empty or whitespace only
    it("throws error on empty input", () => {
        expect(() => new CodeblockParser("   \n   \n", MOCK_CONTEXT)).toThrow("Codeblock is empty or has only whitespaces.");
    });

    // Throws if codeblock starts without pair
    it("throws error if codeblock starts without pair", () => {
        expect(() => new CodeblockParser("no-colon here", MOCK_CONTEXT)).toThrow(/Codeblock must start with a pair/i);
    });

    // Throws if codefence starts and never ends
    it("throws error if codefence is not closed", () => {
        expect(() => new CodeblockParser(MOCKS.codefenceNotClosed, MOCK_CONTEXT)).toThrow(/Unexpected end of input/i);
    });
    ...
    it("parses codefence with 4 backticks", () => {
        const parser = new CodeblockParser(MOCKS.codefence4Backticks, MOCK_CONTEXT);
    ...
    it("parses pair with colon in value", () => {
        const parser = new CodeblockParser(MOCKS.pairWithColonInValue, MOCK_CONTEXT);
    ...
    it("parses codefence with language annotation as text", () => {
        const parser = new CodeblockParser(MOCKS.codefenceWithLang, MOCK_CONTEXT);
        expect(parser.pairList.find(p => p.key === "example")?.value)
            .toBe('with lang\n```js\nconsole.log("Hello");\n```');
    });
    ...
    it("parses pair with hyphenated name", () => {
        const parser = new CodeblockParser(MOCKS.pairHyphenatedName, MOCK_CONTEXT);
    ...
    it("parses pair and ignores trailing empty lines", () => {
        const parser = new CodeblockParser(MOCKS.trailingEmptyLines, MOCK_CONTEXT);
    ...
    it("parses multiple code blocks in different pairs", () => {
        const parser = new CodeblockParser(MOCKS.multipleCodeBlocksDifferentProps, MOCK_CONTEXT);
    ...
    it("parses pair with whitespace-only value", () => {
        const parser = new CodeblockParser(MOCKS.whitespaceOnlyValue, MOCK_CONTEXT);
    ...
    it("throws an error for a single duplicate pair", () => {
        expect(() => new CodeblockParser(MOCKS.duplicateSingle, MOCK_CONTEXT))
    ...
    it("throws an error listing multiple duplicate pairs", () => {
        expect(() => new CodeblockParser(MOCKS.duplicateMultiple, MOCK_CONTEXT))
    ...
    it("does not throw when all pairs are unique", () => {
        expect(() => new CodeblockParser(MOCKS.uniqueProperties, MOCK_CONTEXT))
            .not.toThrow();
    });
});
